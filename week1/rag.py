...
# TODO: Fill this in!
YOUR_SYSTEM_PROMPT = """You are a careful assistant that follows instructions exactly.
...
Rules:
- Use ONLY the provided context for factual details like Base URL, endpoints, headers, and response JSON.
- If required context is missing, output a single fenced Python code block that raises a ValueError explaining what is missing.
- Output must be exactly one fenced Python code block and nothing else.
- Use the requests library.
- For HTTP errors, call response.raise_for_status().
...
def YOUR_CONTEXT_PROVIDER(corpus: List[str]) -> List[str]:
    ...
    keywords = ["Base URL", "X-API-Key", "GET /users/", "Returns 200", "JSON"]
    scored: List[tuple[int, str]] = []
    for doc in corpus:
        if not doc or doc.startswith("[missing_file]") or doc.startswith("[load_error]"):
            continue
        score = sum(1 for k in keywords if k.lower() in doc.lower())
